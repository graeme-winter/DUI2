...
def load_img_json_w_str(uni_url=None, nod_num_lst=[1], img_num=0, exp_path=None):
    my_cmd_lst = ["gi " + str(img_num)]
    my_cmd = {"nod_lst": nod_num_lst, "path": exp_path, "cmd_lst": my_cmd_lst}
...
def load_mask_img_json_w_str(uni_url=None, nod_num_lst=[1], img_num=0, exp_path=None):
...
class np2bmp_heat(object):
    ...
    def img_2d_rgb(self, data2d=None, invert=False, i_min_max=[None, None]):
        ...
        if invert == True:
            for x in np.nditer(
                data2d_scale[:, :], op_flags=["readwrite"], flags=["external_loop"]
            ):
                x[...] = 764.0 - x[...]
        ...
        for x in np.nditer(
            img_array_r[:, :], op_flags=["readwrite"], flags=["external_loop"]
        ):
            x[...] = self.red_byte[x]

        img_array_g[:, :] = scaled_i[:, :]
        for x in np.nditer(
            img_array_g[:, :], op_flags=["readwrite"], flags=["external_loop"]
        ):
            x[...] = self.green_byte[x]

        img_array_b[:, :] = scaled_i[:, :]
        for x in np.nditer(
            img_array_b[:, :], op_flags=["readwrite"], flags=["external_loop"]
        ):
            x[...] = self.blue_byte[x]
...
class np2bmp_monocrome(object):
    ...
    def img_2d_rgb(self, data2d=None, invert=False, i_min_max=[None, None]):
        ...
        for x in np.nditer(
            img_all_chanl[:, :], op_flags=["readwrite"], flags=["external_loop"]
        ):
